...
def calculate_a(planets: [Planet], delta_x: [float] = [], delta_y: [float] = [], k=0):
    """
    ...
    for first_planet in planets:
        for second_planet in planets:
            ...
            fx = gravity * second_planet.m * dx * r_2 ** 3
            fy = gravity * second_planet.m * dy * r_2 ** 3

            fx -= K * first_planet.vx
            fy -= K * first_planet.vy
...
def calculate_energy_and_mass(planets):
    ...
    for first_planet in planets:
        m += first_planet.m  # общая масса
        m_x += first_planet.m * first_planet.x  # координаты общего центра масс
        ...
        for second_planet in planets:
            ...
            dy = second_planet.y - first_planet.y
            r_2 = 1.0 / ((dx * dx + dy * dy) ** 0.5)
            e += first_planet.m * second_planet.m * r_2  # общая энергия
    e = - e * gravity / 2  # todo Поссмотреть формулу
    m_vx = m_x / m
    m_vy = m_y / m
    return e, m_vx, m_vy
...
def calculate(n: int,
    ...
    if schema == 'Euler':
        energy, mass_vx, mass_vy = calculate_by_euler(planets_list, dt, d, k)
    else:
